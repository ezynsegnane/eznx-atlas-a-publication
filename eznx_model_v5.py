...
class EZNX_ATLAS_A_v5(nn.Module):
    ...
    def __init__(
        ...
        self.demo_encoder = MetaMLP_v5(4, hid=64, out_dim=64, dropout=0.10)
        self.anthro_encoder = MetaMLP_v5(12, hid=96, out_dim=64, dropout=0.10)
        self.meta_fuse = nn.Sequential(
            nn.Linear(130, meta_out),
            nn.ReLU(),
    ...
    def forward(self, x_ts, x_meta, meta_present_mask=None):
        ...
        if use_meta:
            ...
            demo_values = x_meta[:, :2]
            demo_mask = meta_present_mask[:, :2]
            anthro_values = x_meta[:, 2:]
            anthro_mask = meta_present_mask[:, 2:]
