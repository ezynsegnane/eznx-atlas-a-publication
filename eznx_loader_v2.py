# eznx_loader_v2.py - PTB-XL dataset loader
import ast
...
class EZNXDataset(Dataset):
    def __init__(self, index_file, data_root, fold=None, sampling_rate=100, meta_mode="demo+anthro"):
        ...
        # Load raw metadata tensors before applying ablations.
        ...
        # 6. Apply the requested metadata ablation mode.
        if meta_mode == "none":
            ...
            # Step 1: neutralize anthropometric values (height, weight, BMI).
            ...
            # Step 2: force missingness flags to 1.0 to mark the ablation.
            ...
            # Step 3: disable the attention mask for ablated columns.
            # Garder actifs seulement les indices 0 et 1 (Age et Sex)
            self.mask[:, 0] = 1.0  # mask__age (actif)
            self.mask[:, 1] = 1.0  # mask__sex (actif)
