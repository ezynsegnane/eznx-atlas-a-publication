...
class ECGAugmentation:
    ...
    def mixup(x1, x2, y1, y2, alpha=0.4):
        """Mixup entre deux échantillons ECG"""
...
def collate_fn_augmented(items):
    ...
    if np.random.rand() > 0.5:  # 50% du temps
        # Gaussian noise
...
class Config:
    data_root: str = field(
        default_factory=lambda: os.environ.get(
            "PTBXL_DATA_ROOT", str(PROJECT_ROOT / "ptb-xl" / "1.0.3")
        )
    )
    index_path: str = field(
        default_factory=lambda: str(PROJECT_ROOT / "data" / "index_complete.parquet")
    )
    runs_dir: str = field(default_factory=lambda: str(PROJECT_ROOT / "runs"))
    seed: int = 2026
    ...
    gradient_accumulation_steps: int = 2  # Simule batch_size=64
    
    # Gradient clipping
    max_grad_norm: float = 1.0
    
    # Recherche centree sur les zones qui preservent la baseline ECG.
    blend_candidates: Tuple[float, ...] = (0.0, 0.5, 0.65, 0.75, 0.85, 0.95, 1.0)
...
# ----------------------------- Main ----------------------------------------
def main():
    parser = argparse.ArgumentParser(description='Entraînement EZNX_ATLAS_A v5.0 OPTIMISÉ')
    parser.add_argument('-variant', '--variant', type=str, default='demo+anthro', 
                       choices=['none', 'demo', 'demo+anthro'],
                       help='Variante d\'ablation')
    parser.add_argument('--seed', type=int, default=None,
                       help='Random seed (defaults to Config.seed)')
    parser.add_argument('--data_root', type=str, default=None,
                       help='Path to the PTB-XL waveform directory')
    ...
    run_dir = Path(cfg.runs_dir) / f"ATLAS_A_v5_{variant}"
    run_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 80)
    print(f"Data root: {cfg.data_root}")
    print(f"ENTRAÎNEMENT EZNX_ATLAS_A v5.0 OPTIMISÉ - Variante: '{variant}'")
    print("=" * 80)
    print(f"Données: {cfg.index_path}")
    ...
    train_datasets = [
        EZNXDataset(index_file=cfg.index_path, data_root=cfg.data_root, fold=f,
                    sampling_rate=cfg.sampling_rate, meta_mode=variant)
        for f in range(1, 9)
    ]
    train_ds = ConcatDataset(train_datasets)
    
    val_ds = EZNXDataset(index_file=cfg.index_path, data_root=cfg.data_root, fold=9,
                         sampling_rate=cfg.sampling_rate, meta_mode=variant)
    
    test_ds = EZNXDataset(index_file=cfg.index_path, data_root=cfg.data_root, fold=10,
                          sampling_rate=cfg.sampling_rate, meta_mode=variant)

    # Augmentation dans train loader
    train_loader = DataLoader(train_ds, batch_size=cfg.batch_size, shuffle=True, 
                             collate_fn=collate_fn_augmented, num_workers=2, pin_memory=True)
    val_loader = DataLoader(val_ds, batch_size=cfg.batch_size, collate_fn=collate_fn_val,
                           num_workers=2, pin_memory=True)
    test_loader = DataLoader(test_ds, batch_size=cfg.batch_size, collate_fn=collate_fn_val,
                            num_workers=2, pin_memory=True)
    ...
    print("\n[2/5] Initialisation du modèle OPTIMISÉ...")
    
    # Architecture + meta_dropout 
    ...
    from eznx_model_v5 import count_parameters
    ...
    print("\n[4/5] Début de l'entraînement OPTIMISÉ...")
    print("-" * 80)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for batch_idx, (x_ts, x_meta, mpm, y) in enumerate(tqdm(train_loader, 
                                                                  desc=f"Epoch {epoch}/{cfg.epochs}", 
                                                                  ncols=100)):
            x_ts, x_meta, mpm, y = x_ts.to(device), x_meta.to(device), mpm.to(device), y.to(device)
        ...
        # Scheduler step APRÈS chaque epoch (cosine)
        scheduler.step()
        ...
        auc_v_meta = safe_macro_auroc(Yv, val_metrics["p_meta"])
        delta_meta_v = float(val_metrics["delta_meta"])
        
        current_lr = opt.param_groups[0]['lr']

        print(
            f"Epoch {epoch:3d} | Loss: {train_loss/len(train_loader):.4f} | "
            f"AUC_Val: {auc_v:.4f} | F1_Val: {f1_v:.4f} | "
            f"AUC_Fused: {auc_v_fused:.4f} | AUC_ECG: {auc_v_ecg:.4f} | "
            f"AUC_Meta: {auc_v_meta:.4f} | Delta_Meta_Val: {delta_meta_v:+.4f} | "
        ...
        is_tie_with_better_meta = abs(auc_v - best_auc) <= 5e-4 and delta_meta_v > best_delta_meta + 1e-6
        if is_better_auc or is_tie_with_better_meta:
            ...
            checkpoint_path = run_dir / f"best_model_v5_{variant}.pt"
            torch.save({
                "epoch": epoch,
            ...
            print(f" Nouveau record AUC: {best_auc:.4f} - Modèle sauvegardé")
        else:
            patience_ctr += 1
            if patience_ctr >= cfg.patience:
                print(f"\n Early stopping après {epoch} epochs (patience={cfg.patience})")
                break
    ...
    ckpt_path = run_dir / f"best_model_v5_{variant}.pt"
    if ckpt_path.exists():
        ckpt = torch.load(ckpt_path, map_location=device)
        model.load_state_dict(ckpt["model_state_dict"])
        best_auc = ckpt.get("best_auc", best_auc)
        best_delta_meta = ckpt.get("best_delta_meta", best_delta_meta)

        thr_final = ckpt.get("thresholds", np.full(len(DS5_LABELS), 0.5))
        w_fused_final = float(ckpt.get("w_fused", 1.0))
        print(f"Utilisation des seuils optimisés sur Validation")
        print(f"   Seuils: {thr_final}")
        print(f"   Poids de fusion retenu: {w_fused_final:.2f}")

        print("\nInférence sur le Test (Fold 10)...")
        ...
        print("\n" + "=" * 80)
        print(f"RÉSULTATS FINAUX v5.1 - Variante: {variant}")
        print("=" * 80)
        print(f"Meilleur AUC Validation (Fold 9): {best_auc:.4f}")
        print(f"Meilleur Delta Meta Validation:   {best_delta_meta:+.4f}")
        print(f"AUC Test (blend sélectionné):     {auc_m:.4f}")
        print(f"AUC Test (ECG only):              {auc_ecg:.4f}")
        print(f"AUC Test (fused only):            {auc_fused:.4f}")
        print(f"AUC Test (méta coupée):           {auc_no_meta:.4f}")
        print(f"Delta AUC méta active:            {delta_meta_auc:+.4f}")
        print(f"F1 Test (seuils optimisés):       {f1_m:.4f}")
        print("=" * 80)
        print(f"{'Classe':<10} | {'AUC Test':<10} | {'F1 Test':<10} | {'Seuil':<10}")
        print("-" * 80)
        for i, lbl in enumerate(DS5_LABELS):
            print(f"{lbl:<10} | {aucs[i]:<10.4f} | {f1s[i]:<10.4f} | {thr_final[i]:<10.4f}")
        print("=" * 80)

        print(f"\n Modèle sauvegardé: {ckpt_path}")
        print("=" * 80)
    else:
